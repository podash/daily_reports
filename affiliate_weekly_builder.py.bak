...
def _build_block1(rows_ftd: list[dict], rows_dep: list[dict], periods: dict | None = None) -> str:
    ...
    sub_dep = (
        ...
        'margin:16px 0 4px 0;">Top 10 by Deposits</span>'
    )
    body = (
...
def _build_block6(no_ftd: list[dict], low_conv: list[dict]) -> str:
    body = (
        ...
        '<span class="badge b-gray" style="margin-left:8px;">Regs &gt; 30, FTD = 0 (last 4w)</span>'
...
def _build_block7(stats: dict) -> str:
    ...
    th = (
        '<th style="text-align:left;font-size:11px;color:#6b7280;text-transform:uppercase;">Group</th>'
        '<th class="r" style="font-size:11px;color:#6b7280;text-transform:uppercase;">12M Base</th>'
        '<th class="r" style="font-size:11px;color:#9ca3af;text-transform:uppercase;">All</th>'
        '<th class="r" style="font-size:11px;color:#6b7280;text-transform:uppercase;">Active</th>'
        '<th class="r" style="font-size:11px;color:#6b7280;text-transform:uppercase;">% Active</th>'
    )
        ...
        for manager in manager_list:
            for m_label, m_key, is_usd in metrics:
                ...
                for w in weeks_str:
                    ...
                    val = _v(rd.get(m_key))
                    cells += f'<td class="r">{"" if not rd else (_fmt_usd(val) if is_usd else _fmt_int(val))}</td>'
                html += (
                    f'<tr class="{tr_cls}">'
...
def _build_block9(data: dict, periods: dict) -> str:
    ...
    for group in ("Inbound", "Acquisition", "Total"):
        ...
        total_style = ' style="border-top:2px solid #1e3a5f;font-weight:700;background:#f0f4ff;"' if is_total else ""
    ...
    note = (
        ...
        f'Projected = MTD / {days_elapsed} days &times; {days_in_month} days in month. '
        f'vs {_mlabel(p1s)} = (Projected &minus; {_mlabel(p1s)}) / {_mlabel(p1s)}. '
        f'YoY % = (Projected &minus; {_mlabel(yoy_s)} full) / {_mlabel(yoy_s)} full.</p>'
    )
