...
def fetch_manager_affiliate_ids(manager_group: str) -> list[int]:
    ...
    Uses exact match after stripping (inbound)/(acquisition) suffixes.
    """
    rows = execute_query(
        r"""
        ...
        WHERE LOWER(TRIM(REGEXP_REPLACE(
                REGEXP_REPLACE(affiliate_group, '\s*\([^)]+\)\s*$', '', 'i'),
                '\s+', ' ', 'g'
              ))) = %s
...
def load_manager_names() -> dict[str, str]:
    ...
    try:
        ...
        return {k.lower().strip(): v for k, v in raw.items() if v}
...
_AFF_INFO_CTE = r"""
    aff_info AS (
        SELECT
            ...
            TRIM(REGEXP_REPLACE(affiliate_group, '\s*\([^)]+\)\s*$', '', 'i'))        AS manager,
            CASE
...
def fetch_block2_falling_partners(periods: dict, focus_geo: list[str], affiliate_ids: list[int] | None = None) -> list[dict]:
    ...
    Active = COUNT(DISTINCT player) who deposited in the period (regular + mobile cash).
    Drop = curr - ref_avg_week. Sorted by absolute drop (biggest loss first).
    """
    clause_bu = _aff_clause(affiliate_ids, alias="bu")
    extra     = [affiliate_ids] if affiliate_ids else []
    return execute_query(
        f"""
        WITH {_AFF_INFO_CTE},
        -- unique depositors current week (regular)
        curr_reg AS (
            SELECT bu.affiliate_id,
                   bd.parent_id                          AS user_id,
                   bd.converted_amount
            FROM public.bet_deposits bd
            JOIN public.bet_users bu ON bd.parent_id = bu.bnu_id
            WHERE bd.status = 'OK' AND bd.converted_amount > 0
              AND bd.date_only BETWEEN %s AND %s
              AND bu.affiliate_id > 0
              {clause_bu}
        ),
        -- unique depositors current week (mobile cash)
        curr_mc AS (
            SELECT bu.affiliate_id,
                   dc.user_id,
                   dc.converted_amount
            FROM public.bet_add_info_deposit_cash dc
            JOIN public.bet_bets bb ON dc.bet_bnu_id = bb.bnu_id
            JOIN public.bet_users bu ON dc.user_id = bu.bnu_id
            WHERE dc.converted_amount > 0
              AND bb.date::date BETWEEN %s AND %s
              AND bu.affiliate_id > 0
              {clause_bu}
        ),
        current_p AS (
            SELECT affiliate_id,
                   COUNT(DISTINCT user_id)                   AS players_curr,
                   ROUND(SUM(converted_amount)::numeric, 0)  AS deposits_curr
            FROM (SELECT * FROM curr_reg UNION SELECT affiliate_id, user_id, converted_amount FROM curr_mc) c
            GROUP BY affiliate_id
        ),
        -- unique depositors reference 90 days (regular)
        ref_reg AS (
            SELECT bu.affiliate_id, bd.parent_id AS user_id
            FROM public.bet_deposits bd
            JOIN public.bet_users bu ON bd.parent_id = bu.bnu_id
            WHERE bd.status = 'OK' AND bd.converted_amount > 0
              AND bd.date_only BETWEEN %s AND %s
        ...
        -- unique depositors reference 90 days (mobile cash)
        ref_mc AS (
            SELECT bu.affiliate_id, dc.user_id
            FROM public.bet_add_info_deposit_cash dc
            JOIN public.bet_bets bb ON dc.bet_bnu_id = bb.bnu_id
            JOIN public.bet_users bu ON dc.user_id = bu.bnu_id
            WHERE dc.converted_amount > 0
        ...
        reference_p AS (
            SELECT affiliate_id,
                   ROUND(COUNT(DISTINCT user_id)::numeric / 90 * 7, 1) AS players_ref
            FROM (SELECT * FROM ref_reg UNION SELECT * FROM ref_mc) r
            GROUP BY affiliate_id
        )
        SELECT
            c.affiliate_id,
        ...
        """,
        tuple(
            [periods["week4_start"], periods["period_end"]] + extra +  # curr_reg
            [periods["week4_start"], periods["period_end"]] + extra +  # curr_mc
            [periods["ref_start"],   periods["ref_end"]]   + extra +   # ref_reg
            [periods["ref_start"],   periods["ref_end"]]   + extra +   # ref_mc
...
def fetch_block3_new_partners(periods: dict, focus_geo: list[str], affiliate_ids: list[int] | None = None) -> list[dict]:
    clause = _aff_clause(affiliate_ids, alias="")
    extra  = [affiliate_ids] if affiliate_ids else []
    return execute_query(
        f"""
        WITH {_AFF_INFO_CTE},
        first_seen AS (
            SELECT affiliate_id, MIN(stat_date) AS first_date
        ...
        current_p AS (
            SELECT
                ...
                SUM(ftd_count)                                  AS ftd,
                ROUND(SUM(new_players_deposit_sum)::numeric, 0) AS ftd_amount,
                ROUND(SUM(deposits_amount_usd)::numeric, 0)     AS deposits_usd,
                CASE WHEN SUM(registrations_count) > 0
                     THEN ROUND((SUM(ftd_count)::numeric
                          / SUM(registrations_count) * 100), 1)
                     ELSE 0 END AS conversion_pct
            FROM aggregates.affiliate_daily_stats
            WHERE stat_date BETWEEN %s AND %s
              {clause}
            GROUP BY affiliate_id
            HAVING SUM(ftd_count) > 0
        )
        SELECT
            c.affiliate_id,
            COALESCE(ai.manager, '—')    AS manager,
...
def fetch_block4_reactivations(periods: dict, focus_geo: list[str], affiliate_ids: list[int] | None = None) -> list[dict]:
    ...
    return execute_query(
        ...
        reference_p AS (
            SELECT
                affiliate_id,
                ROUND((SUM(ftd_count)::numeric / 90 * 28), 2) AS ftd_ref_norm
            FROM aggregates.affiliate_daily_stats
            WHERE stat_date BETWEEN %s AND %s
              {clause}
            GROUP BY affiliate_id
...
def fetch_block5_zero_activity(periods: dict, focus_geo: list[str], affiliate_ids: list[int] | None = None) -> list[dict]:
    ...
    return execute_query(
        ...
        historical AS (
            SELECT
                affiliate_id,
                ROUND(SUM(deposits_amount_usd)::numeric, 0) AS hist_deposits
            FROM aggregates.affiliate_daily_stats
            WHERE stat_date BETWEEN %s AND %s
              {clause}
            GROUP BY affiliate_id
            HAVING SUM(deposits_amount_usd) > 0
        ),
        current_p AS (
            SELECT affiliate_id, SUM(deposits_amount_usd) AS curr_deposits
            FROM aggregates.affiliate_daily_stats
            WHERE stat_date BETWEEN %s AND %s
              {clause}
            GROUP BY affiliate_id
...
def fetch_block6_no_ftd(periods: dict, focus_geo: list[str], affiliate_ids: list[int] | None = None) -> list[dict]:
    ...
    no_ftd_rows = execute_query(
        ...
        WHERE stat_date BETWEEN %s AND %s
          {clause}
        GROUP BY affiliate_id
        HAVING SUM(registrations_count) > 30
           AND SUM(ftd_count) = 0
        ORDER BY regs DESC
        LIMIT %s
        """,
...
def fetch_block6_low_conversion(periods: dict, focus_geo: list[str], affiliate_ids: list[int] | None = None) -> list[dict]:
    ...
    return execute_query(
        ...
        geo_avg AS (
            SELECT
                country,
                ROUND(SUM(ftd)::numeric / NULLIF(SUM(regs), 0) * 100, 2) AS avg_conv_pct
            FROM partner_stats
            GROUP BY country
        )
        SELECT
        ...
        LIMIT %s
        """,
        tuple([periods["week4_start"], periods["period_end"]] + extra + [LOW_CONV_LIMIT]),
    )
...
def fetch_block7_activity(periods: dict, focus_geo: list[str], affiliate_ids: list[int] | None = None) -> dict:
    ...
    clause     = _aff_clause(affiliate_ids, alias="")
    clause_ap  = _aff_clause(affiliate_ids, alias="ap")
    extra      = [affiliate_ids] if affiliate_ids else []
    ...
    all_rows = execute_query(
        ...
        FROM aff_info ai
        WHERE True
          {clause_ap.replace('ap.affiliate_id', 'ai.affiliate_id') if affiliate_ids else ''}
    ...
    base_rows = execute_query(
        ...
        SELECT
            COALESCE(ai.group_type, 'Other')    AS group_type,
            COUNT(DISTINCT ads.affiliate_id)    AS cnt
        FROM (
            SELECT affiliate_id
            FROM aggregates.affiliate_daily_stats
            WHERE stat_date BETWEEN %s AND %s
    ...
    active_rows = execute_query(
        ...
        SELECT
            ...
            COUNT(DISTINCT ads.affiliate_id)    AS cnt
        FROM (
            SELECT affiliate_id, SUM(deposits_amount_usd) AS deps
            FROM aggregates.affiliate_daily_stats
            WHERE stat_date BETWEEN %s AND %s
              {clause}
            GROUP BY affiliate_id
            HAVING SUM(deposits_amount_usd) >= 100
        ) ads
        LEFT JOIN aff_info ai ON ai.affiliate_id = ads.affiliate_id
        GROUP BY ai.group_type
        """,
    ...
    trend_rows = execute_query(
        ...
        FROM (
            SELECT affiliate_id, stat_date, SUM(deposits_amount_usd) AS deps
            FROM aggregates.affiliate_daily_stats
            WHERE stat_date BETWEEN %s AND %s
              {clause}
            GROUP BY affiliate_id, stat_date
        ) ads
        LEFT JOIN aff_info ai ON ai.affiliate_id = ads.affiliate_id
        WHERE ads.deps >= 100
...
def fetch_block8_totals_by_week(periods: dict, focus_geo: list[str]) -> list[dict]:
    """
    ...
    return execute_query(
        ...
        WHERE aws.week_start BETWEEN %s AND %s
        GROUP BY aws.week_start, ai.manager, ai.group_type
        ORDER BY aws.week_start, ai.manager, ai.group_type
        """,
        (p_start, p_end),
    )
...
def fetch_block9_month_comparison(periods: dict, affiliate_ids: list[int] | None = None) -> dict:
    ...
    Plan-to-date = prev_month_total * (days_elapsed / days_in_month).
    """
    ...
    def _fetch_month(start: date, end: date) -> list[dict]:
        return execute_query(
            ...
            new_players_reg AS (
                ...
                FROM public.bet_deposits bd
                JOIN public.bet_users bu ON bd.parent_id = bu.bnu_id
                WHERE bd.is_firstdep = true AND bd.status = 'OK'
                  AND bd.converted_amount > 0 AND bu.affiliate_id > 0
                  AND bd.date_only BETWEEN %s AND %s
                  {clause_bu}
            ),
            new_players_mc AS (
                SELECT bu.affiliate_id, mc.user_id
                FROM aggregates.mobile_cash_ftd_deposits mc
                JOIN public.bet_users bu ON mc.user_id = bu.bnu_id
                WHERE mc.is_ftd = true AND bu.affiliate_id > 0
                  AND mc.deposit_date BETWEEN %s AND %s
                  {clause_bu}
            ),
            new_players AS (
                SELECT affiliate_id, user_id FROM new_players_reg
                UNION
                SELECT affiliate_id, user_id FROM new_players_mc
            ),
            np_reg AS (
                SELECT bu.affiliate_id, SUM(bd.converted_amount) AS dep_sum
                FROM public.bet_deposits bd
                JOIN public.bet_users bu ON bd.parent_id = bu.bnu_id
                JOIN new_players np ON bd.parent_id = np.user_id
                    AND bu.affiliate_id = np.affiliate_id
                WHERE bd.status = 'OK' AND bd.converted_amount > 0
                  AND bd.date_only BETWEEN %s AND %s
                GROUP BY bu.affiliate_id
            ),
            np_mc AS (
                SELECT bu.affiliate_id, SUM(dc.converted_amount) AS dep_sum
                FROM public.bet_add_info_deposit_cash dc
                JOIN public.bet_bets bb ON dc.bet_bnu_id = bb.bnu_id
                JOIN public.bet_users bu ON dc.user_id = bu.bnu_id
                JOIN new_players np ON dc.user_id = np.user_id
                    AND bu.affiliate_id = np.affiliate_id
                WHERE dc.converted_amount > 0
                  AND bb.date::date BETWEEN %s AND %s
                GROUP BY bu.affiliate_id
            ...
            np_dep_agg AS (
                SELECT affiliate_id,
                       ROUND(SUM(dep_sum)::numeric, 0) AS np_dep_sum
                FROM (SELECT * FROM np_reg UNION ALL SELECT * FROM np_mc) c
                GROUP BY affiliate_id
            ),
            aff_stats AS (
            ...
            SELECT
                COALESCE(ai.group_type, 'Other')        AS group_type,
                SUM(ast.ftd)                             AS ftd,
                COALESCE(SUM(nda.np_dep_sum), 0)         AS ftd_amount,
                SUM(ast.deposits_usd)                    AS deposits_usd
            FROM aff_stats ast
            LEFT JOIN aff_info ai  ON ai.affiliate_id  = ast.affiliate_id
            LEFT JOIN np_dep_agg nda ON nda.affiliate_id = ast.affiliate_id
            GROUP BY ai.group_type
            ORDER BY ai.group_type
            """,
            tuple(
                [start, end] + extra +   # new_players_reg
                [start, end] + extra +   # new_players_mc
                [start, end] +           # np_reg
                [start, end] +           # np_mc
                [start, end] + extra     # aff_stats
            ),
        )
...
def collect_weekly_metrics(report_date: date, focus_geo: list[str], manager_group: str | None = None) -> dict:
    ...
    block8 = enrich_rows(_mgr_filter(fetch_block8_totals_by_week(periods, focus_geo)), names)
